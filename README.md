...
| `lsp-ltex-plus-latex-commands` | L | LaTeX commands to be handled by the LaTeX parser (listed with empty arguments, e.g. `\ref{}`). *Type:* alist of `(command . action)`, where command is a symbol (`\\ref{}` instead of `"\\ref{}"`) and action is `"default"`, `"ignore"`, `"dummy"`, `"pluralDummy"`, or `"vowelDummy"`; *default:* `nil`. | X | |
