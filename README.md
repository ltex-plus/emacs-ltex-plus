...
Four settings are deliberately left for you to confirm, because a silent value would do more than change how your text is checked: `lsp-ltex-plus-lt-server-uri` names the host every document is sent to; `lsp-ltex-plus-lt-username` and `lsp-ltex-plus-lt-api-key` are credentials; and `lsp-ltex-plus-additional-rules-language-model` names a directory the server reads from. The four project *file* settings are vouched for only when the path is relative and free of `..` — one that cannot lead outside the project. An absolute path (or one starting with `~`) still works there, it just asks first. Between them, these mean that cloning someone's repository cannot quietly redirect where your prose is sent, nor where your word lists are read from and written to.
