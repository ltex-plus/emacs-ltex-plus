...
> - **A** — *Activation-time*: read when `lsp-ltex-plus-mode` turns on in a buffer — neither on every check nor once at setup. A buffer already being checked keeps the value it started with; newly opened buffers, and ones where you toggle the mode off and on again, see the new one. No reload or restart is involved. If you changed the value in a `.dir-locals.el`, revert the buffer instead (`M-x revert-buffer`, or `C-x x g` on Emacs 28+): toggling the mode re-reads the variable but not the file it came from.
>
