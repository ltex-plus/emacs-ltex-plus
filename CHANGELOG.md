...
### Added
- **Incremental-checking settings `lsp-ltex-plus-max-fragment-size` and `lsp-ltex-plus-fragment-cache-ttl-minutes`.** These surface the two options added by `ltex-ls-plus` [PR #176](https://github.com/ltex-plus/ltex-ls-plus/pull/176), which makes the server re-check only the paragraphs you edited instead of the whole document. `lsp-ltex-plus-max-fragment-size` (default `20000`, the per-request character limit of the free remote LanguageTool service) is the largest amount of text in characters sent to LanguageTool in a single request when a run of changed paragraphs is batched together — larger text is split across several requests, but an individual paragraph is never split, and caching granularity stays per paragraph. `lsp-ltex-plus-fragment-cache-ttl-minutes` (default `30`) is how long a document's cached results are kept after they stop being used before a background sweep drops them. Both are wired through `lsp-register-custom-settings` (`ltex.maxFragmentSize`, `ltex.fragmentCacheTtlMinutes`) and the `:initialized-fn` configuration snapshot, so they are read live on every `workspace/configuration` pull with no restart needed.
